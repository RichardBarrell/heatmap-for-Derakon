...
static void setbit(u8 *u, int val, ii x, ii y, ii xMax) {
	ii pos = xMax * x + y;
	ii pos_byte = pos >> 3;
	ii pos_bit  = pos & 0x7;
	u8 bit = 1 << pos_bit;
	u[pos_byte] = (u[pos_byte] & ~bit) | (val ? bit : 0);
}

void burnHeatMap(ii xMax, ii yMax,
		 int32_t *heatMap,
		 size_t goals_length,
		 ii *goals_xs,
		 ii *goals_ys)
	...
	while (queue_used(&cell_todo) > 0) {
		...
		for (ii yi = MAX(0, y-1); yi < MIN(yMax, y+2); yi++) {
			for (ii xi = MAX(0, x-1); xi < MIN(xMax, x+2); xi++) {
				if (GETUSED(xi, yi)) { continue; }
				SETUSED(xi, yi, 1);
				IX(xi, yi) = cost + 1;
				pair xiyi = {xi, yi};
				queue_append(&cell_todo, xiyi);
			}
		}
	}
#ifdef TESTY
...
int main(int argc, char **argv) {
	...
	ii goal_x = 4;
	ii goal_y = 8;
	burnHeatMap(HI, WI, ra, 1, &goal_x, &goal_y);
	return 0;
}
#endif
